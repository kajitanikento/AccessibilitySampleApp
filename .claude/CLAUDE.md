...
詳細ルール: `.claude/design-system.md` を必ず参照すること。
...
AccessibilitySampleApp/
  ...
  ├── TaskModel.swift          # TodoTask, Priority, TaskStore
  ├── TaskListView.swift       # タスク一覧・AddTaskSheet・FilterSheetView
  ├── TaskDetailView.swift     # タスク詳細（Push遷移）
  ├── WCAGDemoView.swift       # WCAG 4原則デモ
  └── DesignSystem/            # デジタル庁デザインシステム実装
```

## 開発上の注意

- `PBXFileSystemSynchronizedRootGroup` 使用のため、`.swift` 追加時にpbxproj変更不要
- Swift の `Task` 型と衝突するため独自モデルは `TodoTask` と命名
- SourceKit-LSP は別ファイルのシンボルを "Cannot find in scope" と誤報することがある（Xcodeビルドでは正常）
- `Color(UIColor.systemGray5)` など UIColor ベースの初期化は型を明示すること
